...
class Index extends BaseController
{
    ...
    public function index()
    {
        ...
        $custom_template_info = $this->getCustoTemplate();
     ...
     * 获取自定义模板信息
     */
    public function getCustoTemplate(){
        $config = new Config();
        //自定义模板查询
            ...
            foreach($custom_template_info as $k=>$v){
                if($v["control_name"] == "GoodsList"){
                    ...
                    if($v["style_data"]["goods_source"] > 0){
                        $goods_list = $goods->getGoodsList(1, $v["style_data"]["goods_limit_count"] , ["ng.category_id"=>$v["style_data"]["goods_source"]], "ng.create_time desc");
                        $goods_query = array();
                        if(!empty($goods_list)){
